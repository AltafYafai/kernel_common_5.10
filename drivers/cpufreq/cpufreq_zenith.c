...
#include <linux/sched/cpufreq.h> /* Crucial for schedutil hooks */
...
struct zenith_policy {
    ...
    unsigned int up_threshold;    /* Ondemand style threshold (e.g., 80%) */
...
/* * The Schedutil Hook: This is called DIRECTLY by the scheduler 
 * whenever task utilization changes. Zero latency.
 */
static void zenith_update_util(struct update_util_data *data, u64 time, unsigned int flags)
{
    ...
    unsigned int util, max, next_freq;

    /* Rate limit to avoid choking the CFS scheduler (e.g., 1ms min delay) */
    if (time_before64(time, z_policy->last_update + NSEC_PER_MSEC))
        return;
    z_policy->last_update = time;

    /* * [Reflex Logic Injection Point]
     * If SCHED_CPUFREQ_IOWAIT or SCHED_CPUFREQ_RT is flagged, 
     * instantly spike to max frequency.
     */
    ...
    /* * [Schedutil + Ondemand Math Point]
     * In a full implementation, you would read 'util' from the CPU runqueue.
     * Here, we simulate the Ondemand jump: if util > up_threshold, spike it.
     * Otherwise, decay it smoothly.
     */
     
    // Placeholder for actual CFS util extraction
    // util = cpu_util_cfs(smp_processor_id()); 
    // max = arch_scale_cpu_capacity(smp_processor_id());

    /* For the skeleton, we keep it floating mid-way unless pushed */
    next_freq = policy->min + ((policy->max - policy->min) / 2);

set_freq:
    if (z_policy->freq_target != next_freq) {
        z_policy->freq_target = next_freq;
        /* Fast-path frequency switch */
...
/* * zenith_start: Registers the scheduler hook to start receiving live updates. */
static int zenith_start(struct cpufreq_policy *policy)
{
    ...
    /* Attach to the scheduler's utilization callback framework */
...
static void zenith_stop(struct cpufreq_policy *policy)
{
    ...
    synchronize_rcu(); /* Ensure all scheduler callbacks are finished */
...
{
    /* Handle min/max freq changes from user space */
