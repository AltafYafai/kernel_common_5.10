...
struct zios_rq_data {
	...
	bool from_slab;			/* true: free via kmem_cache_free, false: via kfree */
};
...
static void zios_prepare_request(struct request *rq)
{
	...
	if (rd) {
		...
		/*
		 * Slab allocation failed under memory pressure.
		 * Fall back to the general allocator rather than
		 * silently dropping the request (which would hang
		 * the issuer indefinitely).
		 */
...
static ssize_t zios_thermal_zone_name_show(struct elevator_queue *e, char *page)
{
	...
	spin_lock(&zd->lock);
	strscpy(buf, zd->thermal_zone_name, sizeof(buf));
	spin_unlock(&zd->lock);
					     ...
					     char *page)
{
	...
	spin_lock(&zd->lock);
	strscpy(buf, zd->top_app_cgroup_name, sizeof(buf));
	spin_unlock(&zd->lock);
	return sysfs_emit(page, "%s\n", buf);
}
